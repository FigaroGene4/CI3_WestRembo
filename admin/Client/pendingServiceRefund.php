...
<body>
  ...
  <div class="sidebar">
    ...
    <a href="refundsandcomplaint.php">Complaints and Refund</a>
    ...
    $result1 = $conn-> query($sql);
     ...
     if($result1->num_rows >0){
        while($row = $result1 -> fetch_assoc()){
          ...
          $check = "SELECT * FROM table_complaint WHERE transactionNumber = '$transactionNumber'";
          $res = mysqli_query($con, $check);

          $check1 = "SELECT * FROM table_refund_request WHERE transactionNumber = '$transactionNumber'";
          ...
          if(mysqli_num_rows($res) == 0 && mysqli_num_rows($res1) ==0 ) {
            ...
            <a class="dropdown-item" href="complaint.php?transactionNumber='.$transactionNumber.'">File a complaint</a>
            <a class="dropdown-item" href="refund.php?transactionNumber='.$transactionNumber.'">Request for a refund</a>
        ...
        ';
          ...
          if(mysqli_num_rows($res) > 0 && mysqli_num_rows($res1) ==0 ) {
            ...
            echo '<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
            <a class="dropdown-item disabled" href="complaint.php?transactionNumber='.$transactionNumber.'">File a complaint</a>
            <a class="dropdown-item" href="refund.php?transactionNumber='.$transactionNumber.'">Request for a refund</a>
        ...
        ';
          ...
          if(mysqli_num_rows($res) ==0 && mysqli_num_rows($res1) > 0 ) {
            ...
            echo '<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
            <a class="dropdown-item " href="complaint.php?transactionNumber='.$transactionNumber.'">File a complaint</a>
            <a class="dropdown-item disabled" href="refund.php?transactionNumber='.$transactionNumber.'">Request for a refund</a>
...
$sql = "SELECT * from table_book where clientEmail = '$email' AND  status = 'Pending'";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    echo "<br><br><h1> Artist: " . $artistFirstName . " " . $artistLastName . ' ' . " </h1>";
...
<option>Artist took to long to accept my booking</option>
...
<option>Others</option>
 ...
 ';
   ...
   echo '<input type="submit" class="btn btn-info"  name="refundz">';
    ...
    if ($result1->num_rows > 0) {
      while ($row = $result1->fetch_assoc()) {
        ...
        $img = $row['img'];
                ...
                echo "Your money will be refunded shorltly.";
...
if (isset($_POST['refundz'])) {
    ...
    $clientFirstName;
    $_SESSION['tsk'] = 'Requset for refund upon review. Transaction Number: ' . $transactionNumber;

   
   
    $insert_data1= "INSERT INTO table_refund_request (transactionNumber, artistEmail, artistFirstName,artistLastName,clientFirstName,clientLastName, amount, date, refundComplaint, categoryRefund) VALUES ('$transactionNumber','$artistEmail', '$artistFirstName','$artistLastName', '$clientFirstName','$clientLastName', '$price', '$date', '$refundContent' ,'$categoryRefund')";
    ...
    $sql1 = "UPDATE table_book SET refundComplaint = '$refundContent', categoryRefund = '$categoryRefund', newStatus ='Refund Request' WHERE transactionNumber = '$transactionNumber'";
            $conn->query($sql1);
